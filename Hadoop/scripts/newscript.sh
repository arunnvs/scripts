  ...
  do
	...
	do
	  ...
	  do
		...
		do
		  sed -e "/mapreduce.map.cpu.vcores/{n;s/.*/\<value\>$mapvcore\<\/value\>/}" -e "/mapreduce.reduce.cpu.vcores/{n;s/.*/\<value\>$redvcore\<\/value\>/}" -e "/mapreduce.map.memory.mb/{n;s/.*/\<value\>$mapmem\<\/value\>/}" -e "/mapreduce.reduce.memory.mb/{n;s/.*/\<value\>$reducemem\<\/value\>/}" -e "/mapreduce.map.output.compress/{n;s/.*/\<value\>$compress\<\/value\>/}" /home/hduser2/scripts/yarn-site.xml_orig > /home/hduser2/scripts/yarn-site.xml
		  sed -e "/dfs.replication/{n;s/.*/\<value\>$replication\<\/value\>/}" -e "/dfs.block.size/{n;s/.*/\<value\>$blocksize\<\/value\>/}"  /home/$user/scripts/hdfs-site.xml_orig > /home/$user/scripts/hdfs-site.xml
		  ...
		  do
			...
			do
				...
				Job1logdir="/home/hduser2/scripts/job_log/"$Job1"_cmprs-"$compress"_block-"$block"MB_redmem-"$reducemem"_mapmem-"$mapmem"_mapvcore-"$mapvcore"_redvcore-"$redvcore"_repln-"$replication"_datavol-"$datavol"GB"
				Job2logdir="/home/hduser2/scripts/job_log/"$Job2"_cmprs-"$compress"_block-"$block"MB_redmem-"$reducemem"_mapmem-"$mapmem"_mapvcore-"$mapvcore"_redvcore-"$redvcore"_repln-"$replication"_datavol-"$datavol"GB"
